...
// ── Navbar Scroll + Scroll Down Arrow ──
...
function showSecurityPopup(message) {
    ...
    popup.innerHTML = `
        <div style="margin-bottom: 20px;">
            ...
            <h3 style="color: white; margin-bottom: 10px; font-family: Inter, sans-serif;">Security Notice</h3>
            <p style="color: var(--gray); line-height: 1.6; font-family: Inter, sans-serif; font-size: 0.95rem;">${message}</p>
        </div>
...
document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'F12' || e.keyCode === 123) {
        e.preventDefault();
        showSecurityPopup('Developer tools are disabled on this page.');
        return false;
    }
    ...
    if ((e.ctrlKey && e.key === 'u') || (e.metaKey && e.key === 'u')) {
        e.preventDefault();
        showSecurityPopup('Viewing page source is disabled on this page.');
        return false;
    }
    // Ctrl+Shift+C / Cmd+Option+C — Inspect Element
    if ((e.ctrlKey && e.shiftKey && e.key === 'C') || (e.metaKey && e.altKey && e.key === 'c')) {
        e.preventDefault();
        showSecurityPopup('Inspect element is disabled on this page.');
        return false;
    }
});
...
setInterval(() => {
    ...
    if (end - start > 100) {
        if (!devtoolsOpen) {
            devtoolsOpen = true;
            showSecurityPopup('Developer tools detected. Please close them to continue browsing.');
        }
    } else {
        devtoolsOpen = false;
    }
